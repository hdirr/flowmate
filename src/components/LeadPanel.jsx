...
import { X, Phone, Mail, MoveRight, Trash2, Save, Plus, ChevronDown, Hash, Calendar, List, Type } from 'lucide-react';
...
export default function LeadPanel({ lead, onClose, onUpdate, onRemove }) {
  ...
  const [customFields, setCustomFields] = useState(() => db.customFields.list());
  ...
  function saveContact() {
    db.contacts.update(lead.contact_id, { ...form, fields: fieldValues });
    onUpdate();
    setEditing(false);
  }

  function moveStage(stageId) {
    db.leads.update(lead.id, { stage_id: stageId });
    onUpdate();
  }

  function remove() {
    if (!confirm('Remover este lead?')) return;
    db.leads.remove(lead.id);
    onRemove();
  }
  ...
  function createField() {
    ...
    const options = newField.type === 'select'
      ...
      : [];
    db.customFields.create({ name: newField.name.trim(), type: newField.type, options });
    setCustomFields(db.customFields.list());
            ...
            <div className="flex justify-between items-center mb-3">
              ...
              {!editing
                ...
                : <div className="flex gap-2">
                    ...
                    <button onClick={saveContact} className="text-xs text-blue-600 font-semibold hover:underline flex items-center gap-1"><Save className="w-3 h-3" />Salvar</button>
                  </div>
              }
            </div>

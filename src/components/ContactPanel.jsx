...
import { X, Phone, Mail, Save, User, KanbanSquare, Plus, Trash2, ChevronDown, Hash, Calendar, List, Type, Loader2 } from 'lucide-react';
...
export default function ContactPanel({ contact, onClose, onSave }) {
  ...
  async function save() {
    ...
    setSaving(true);
    await db.contacts.update(contact.id, { ...form, fields: fieldValues });

    if (stageId) {
      if (existingLead) await db.leads.update(existingLead.id, { stage_id: stageId });
      else await db.leads.create({ contact_id: contact.id, stage_id: stageId });
    } else if (existingLead) {
      await db.leads.remove(existingLead.id);
    }
  ...
  async function removeField(id) {
    ...
    await db.customFields.remove(id);
    const cf = await db.customFields.list();
    setFields(cf);
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-sm bg-white h-full shadow-2xl flex flex-col">
        ...
        <div className="px-5 py-4 border-t border-gray-100">
          <button onClick={save} disabled={!form.name.trim() || saving || loading}
